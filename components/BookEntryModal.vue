<template>
    <div>
        <b-button v-b-modal.bookEntryModal variant="primary" class="float-right mt-2">Add Book</b-button>
        <b-modal id="bookEntryModal" title="Add Book" ok-title="Save Book" @ok="onSubmit">
            <form action="#">
                <b-form-group
                label="Enter title"
                label-for="title"
...
export default {
    ...
    methods: {
        async onSubmit() {
            this.$axios.post('https://lentrix.tk/lmdrl/api/books', this.book)
            .then((res)=>{
                if(res.status==202) {
